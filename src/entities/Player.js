    ...
    update(delta, arena, particles) {
        ...
        if (arena) {
            ...
            if (tile && tile.state === 'BONUS') {
                const effect = tile.powerUpType
                    ? POWER_UP_EFFECTS.find(pu => pu.type === tile.powerUpType)
                    : null;
                const chosenEffect = effect || POWER_UP_EFFECTS[Math.floor(Math.random() * POWER_UP_EFFECTS.length)];
                const settings = useGameStore.getState().settings;
